...
//app dependencies
const _1mDownloadPath = path.join(__dirname, './cron-jainlogic/1m');
const _1yDownloadPath = path.join(__dirname, './cron-jainlogic/1y');
...
(async (response) => {
  ...
  if(process.env.JAINLOGIC_CRON_VERBOSE_LOGGING == true){
    await page.screenshot({ path: './cron-jainlogic/debugging/afterNetworkIdle.png' });
  ...
  if(process.env.JAINLOGIC_CRON_VERBOSE_LOGGING == true){
    await page.screenshot({ path: './cron-jainlogic/debugging/afterClickingView.png' });
